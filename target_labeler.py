...
post_target = set(["ABDUCTED BY", "PERPETRATED BY","ATTACKS ON", "FIRED AT", "SET FIRE", "BOMB PLANTED", "BURNING",
"ATTACKED THE", "ATTACK ON", "ATTACKED", "BLEW UP", "EXPLODED IN", "SABOTAGE AGAINST", "ATTACKS ON", "THREW A BOMB", "EXPLODED"])

pre_target = set(["HAVE PARTICIPATED", "WAS DERAILED", "WERE ARRESTED", "WAS DYNAMITED", "WAS ATTACKED", "DESTROYED", "SHOT AT", "WERE DAMAGED"])
...
def getLeftResult(pos_document_array, endIndices):
    ...
    for endIndex in endIndices:
        for k in range(0, 9):
            ...
            if pos_item[1].startswith('NN') or pos_item[1].startswith('DT') and not pos_item[0] == '[' and not pos_item[0] == ']':# == 'NNP':
