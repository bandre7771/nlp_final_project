...
def get_victim(document, pos_document_array, parse_document_array):
    ...
    for phrase in victim_left:
        index = document.find(phrase)
        if index != -1:
            # if document_array[0] == 'DEV-MUC3-0324':
            #     pdb.set_trace()
            phrase_array = phrase.split()
...
def getLeftResult(pos_document_array, endIndex):
    ...
    startIndex = ""
    for k in range(0, 6):
        if pos_document_array[endIndex - k][1].startswith('NN'):# == 'NNP':
            ...
            while True:
                if pos_document_array[endIndex - k][1] == 'CC':
                    ...
                    # pdb.set_trace()
                    for o in range(0, 6):
                        if pos_document_array[endIndex - o][1].startswith('NN'):# == 'NNP':
                            # result += str(endIndex - o) + ' '
                            while True:
                                if not pos_document_array[endIndex - o][1].startswith('NN'): #!= 'NNP':
                            ...
                            break
                    break
                elif not pos_document_array[endIndex - k][1].startswith('NN'): # != 'NNP':
                    startIndex = str(endIndex - k + 1)
                    result = startIndex + ' ' + result
                    break
        ...
        if pos_document_array[index - j][0] == ',':
            ...
            for z in range(index - 1):
                if pos_document_array[index - z][1].startswith('NNP'): #== 'NNP':
                    while True:
                        if not pos_document_array[index - z][1].startswith('NNP'):# != 'NNP':
                            # pdb.set_trace()
        ...
        if pos_document_array[k + endIndex][1].startswith('NN'):# == 'NNP':
            ...
            while True:
                if pos_document_array[k + endIndex][1] == 'CC':
                    ...
                    # pdb.set_trace()
                    for o in range(0, 6):
                        if pos_document_array[o + endIndex][1].startswith('NN'):# == 'NNP':
                            result += str(o + endIndex)
                            while True:
                                if not pos_document_array[o + endIndex][1].startswith('NN'):# != 'NNP':
                                    break
                                result += ' ' + pos_document_array[o + endIndex][0]
                                o += 1
                            break
                    break
                if not pos_document_array[k + endIndex][1].startswith('NN'):# != 'NNP':
                    break
...
def cleanResult(result, parse_document_array):
    ...
    for r in result_array:
        ...
        for i in range(1, len(result_a)):
            if parse_document_array[index][1] == 'PERSON' or parse_document_array[index][0] == 'LUIS':
                # pdb.set_trace()
                is_person = True
